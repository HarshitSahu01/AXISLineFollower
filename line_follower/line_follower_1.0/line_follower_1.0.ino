...
float Kp = 0.15; //related to the proportional control term; 
              ...
              // Sweet spot: 0.06
float Ki = 0.005; //related to the integral control term; 
              //change the value by trial-and-error (ex: 0.0008).
float Kd = 0.60; //related to the derivative control term; 
...
*************************************************************************/
const uint8_t maxspeeda = 85;
const uint8_t maxspeedb = 85;
...
void setup() {
  qtr.setTypeRC();
  qtr.setSensorPins((const uint8_t[]){A0, 12, 11, 10, 9, 8, 7, A1}, SensorCount);
  Serial.begin(9600);
...
void rotate_right() {
  analogWrite(aphase, rotation_speed);
...
void rotate_left() {
  ...
  analogWrite(aenbl, rotation_speed);
